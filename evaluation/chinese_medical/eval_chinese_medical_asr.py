#!/usr/bin/env python3
"""
Evaluate Qwen3-ASR on Chinese medical TTS speech (yidu_wenet_cosyvoice/voxcpm).
...
Normalization via masr_eval_pkg:
  - ChineseNormalizer: NFKC, number ITN (二零零八 → 2008), punctuation removal
  - MixedTokenizer for MER

Usage:
  python eval_chinese_medical_asr.py \
    --jsonl evaluation/chinese_medical/yidu_wenet_cosyvoice_test.jsonl \
    --model Qwen/Qwen3-ASR-1.7B \
    --batch_size 4 \
    --output_predictions outputs/qwen3_asr_1.7b_cosyvoice/predictions.jsonl \
    --output_summary outputs/qwen3_asr_1.7b_cosyvoice/summary.txt
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Chinese medical ASR eval for Qwen3-ASR.")
    ...
    p.add_argument("--max_new_tokens", type=int, default=1024, help="Long medical reports need more tokens.")
...
def main() -> None:
        ...
        for b_start in range(0, len(todo), args.batch_size):
            ...
            for idx, o in zip(batch_idx, outs):
                ...
                rec = {
                    ...
                    "originalText": ex.get("originalText", ""),
    ...
    summary_lines = [
        "=== Chinese Medical ASR Evaluation ===",
        f"Model:               {args.model}",
